...
const getOneCertificate = async(year, number, name, surname, patronymic) => {
    try {
        ...
        const result = await client.query(`SELECT start_date, end_date, number, year_graduation, comission_number, position FROM certificates WHERE person_fk in (SELECT person_id FROM persons WHERE name = '${name}' AND patronymic = '${patronymic}' AND surname = '${surname}')  AND number = '${number}' AND year_graduation='${year}'`)
        client.end();
